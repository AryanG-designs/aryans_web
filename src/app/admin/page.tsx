        ...
        {!active ? (
          ...
          <div className="space-y-8 rounded-2xl bg-cream p-6 shadow-sm">
            ...
            <div className="border-t border-ink/10 pt-6">
              <p className="mb-3 font-display text-lg font-semibold">The Brief</p>
              <div className="space-y-4">
                <Field label="Problem">
                  <textarea
                    className={inputClass}
                    rows={3}
                    value={active.brief.problem}
                    onChange={(e) => updateActive({ brief: { ...active.brief, problem: e.target.value } })}
                  />
                </Field>
                <Field label="Audience">
                  <input
                    className={inputClass}
                    value={active.brief.audience}
                    onChange={(e) => updateActive({ brief: { ...active.brief, audience: e.target.value } })}
                  />
                </Field>
                <Field label="Constraints">
                  <input
                    className={inputClass}
                    value={active.brief.constraints}
                    onChange={(e) => updateActive({ brief: { ...active.brief, constraints: e.target.value } })}
                  />
                </Field>
                <Field label="Objectives (one per line)">
                  <textarea
                    className={inputClass}
                    rows={4}
                    value={active.brief.objectives.join("\n")}
                    onChange={(e) =>
                      updateActive({
                        brief: { ...active.brief, objectives: e.target.value.split("\n").filter((s) => s.trim()) },
                      })
                    }
                  />
                </Field>
            ...
            <div className="space-y-6 border-t border-ink/10 pt-6">
              <GalleryEditor
                label="Research"
                items={active.research}
                onChange={(items) => updateActive({ research: items })}
              />
              <GalleryEditor
                label="Exploration / Sketching"
                items={active.exploration}
                onChange={(items) => updateActive({ exploration: items })}
              />
              <GalleryEditor
                label="Process / Development"
                items={active.process}
                onChange={(items) => updateActive({ process: items })}
              />
              <GalleryEditor
                label="Final Outcome"
